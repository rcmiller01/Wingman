"""Log Collector - ingests container logs with retention metadata."""
...
class LogCollector:
    ...
    async def get_logs(
        ...
        query = select(Log).where(Log.resource_ref == resource_ref)
    ...
    async def purge_expired_logs(self, db: AsyncSession) -> int:
        ...
        result = await db.execute(
            delete(Log).where(Log.retention_date < now)
        )
